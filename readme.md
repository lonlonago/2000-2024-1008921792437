## Body

（2000-2024年）上市公司-数字技术风险暴露数据|大语言模型

一、数据介绍
（1）数据详情：见下方图片，拍前请看清楚！已完整展示说明，请确定好是不是自己需要的再拍下！发货后任何理由不退不换！
（2）数据名称：上市公司-企业数字技术风险暴露数据、数据风险暴露、网络风险暴露数据
（3）样本范围：A鼓尚视公司,4.5w观测值（未剔除未缩尾）
（4）时间范围：2000-2024年
（5）结果数据格式：excel、dta
（6）数据内容：原始数据、结果（一个版本，未剔除未缩尾）、do文档代码（简易）、python代码（部分计算代码，数据处理非常复杂，需要较强数据处理技术和安装包，因此代码仅供参考不教学）、参考文献

二、数据结果指标如图所示

三、计算方法如图所示

四、参考文献如图所示

## Images

![img_01.jpg](img_01.jpg)
![img_02.jpg](img_02.jpg)
![img_03.jpg](img_03.jpg)
![img_04.jpg](img_04.jpg)
![img_05.jpg](img_05.jpg)
![img_06.jpg](img_06.jpg)
![img_07.jpg](img_07.jpg)

item_1008921792437

Here is a pay link on Stripe ( https://buy.stripe.com/3cs8yP7sY87d0vu9AB ). Please contact me lonlonago@foxmail.com after funding $89, and I will send you a complete data files , thank you!

![111.png](111.png)
